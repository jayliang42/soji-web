const requiredEnv = {
  ...
  GUEST_CHECKOUT_HMAC_SECRET: process.env.GUEST_CHECKOUT_HMAC_SECRET,
  OPS_ALERT_WEBHOOK_URL: process.env.OPS_ALERT_WEBHOOK_URL,
  SOJI_DEMO_MODE: process.env.SOJI_DEMO_MODE,
  SOJI_CHECKOUT_TEST_BROWSER_HMAC:
    process.env.SOJI_CHECKOUT_TEST_BROWSER_HMAC,
...
const trustedProductionSiteOrigins = new Set([
...
export function getCheckoutReturnSiteUrl(
  ...
  if (
    ...
    trustedProductionSiteOrigins.has(requestOrigin.origin) ||
...
export function getClientSiteUrl(
  ...
  if (
    isValidSiteUrl(currentOrigin, nodeEnv) &&
    (nodeEnv !== "production" ||
      trustedProductionSiteOrigins.has(new URL(currentOrigin).origin))
  ) {
...
export function getAuthReturnSiteUrl(
  requestUrl: string,
  configuredValue = env.NEXT_PUBLIC_SITE_URL,
  nodeEnv = process.env.NODE_ENV
) {
  ...
  if (
    isValidSiteUrl(requestOrigin, nodeEnv) &&
    (nodeEnv !== "production" ||
      trustedProductionSiteOrigins.has(requestOrigin))
  ) {
